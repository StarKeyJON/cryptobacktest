...
results = './optimizer_results/EMA_OPT__3.csv'
...
class KeltnerChannel(bt.Strategy):
    ...
    lines = ('mid', 'top', 'bot',)
    params = dict(period=34,devfactor=1.5,fast=7,slow=58,long=253, xlong=553,
              movav= bt.ind.MovAv.Simple,order_count=0,rsi_period=5,lookback=7,
    ...
    def next(self):
            ...
            if self.dataclose > self.long_ema[0]:
                if self.signal2 < 0.0 and self.rsi < 30:
            ...
            # Sell when price below long EMA and fast EMA crosses below slow EMA
                ...
                if self.signal2 < 0.0 and self.in_position > 0 and self.rsi > 60 and self.macd < 0:
...
if __name__ == '__main__':
    ...
    for project in crypto:
        ...
        cerebro.broker.setcommission(commission=.0045)
